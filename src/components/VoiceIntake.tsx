...
import type { SourceInput } from "../platform/host";
import { t } from "../i18n";

type Piece = { key: number; kind: "paste" | "file"; origin: string | null; body: string };
        ...
        {pieces.map((p, i) => (
          <li key={p.key} className="piece">
            <div className="piece-head">
              <span className="muted">{p.kind === "file" && p.origin ? p.origin : t("voice.intake.piece", { n: String(i + 1) })}</span>
