...
0. **おすすめの文体（D-70、#124）— 2026-09-24 完了**（要件 `requirements/2026-09-24-voice-preset.md`（必須 21）、UX 記録 `ux/2026-09-24-voice-preset.md`、試し書き `poc/voice-preset/trial.md`）。`lita_codex::presets`（手書きの `VoiceProfile`「広報のです・ます」`pr-polite-ja`）、コマンド `list_voice_presets` / `create_voice_from_preset`（材料 0 本で複製、名前重複は「(2)」）。入口は「Lita の書き方を決める」の主ボタンの下に薄いリンク「おすすめの文体から始める」＋名前と一文。材料 0 本の文体ページは「0 件」・根拠・確度を出さず、由来の一文だけ。試し書き: 110 秒・2,023 字で契約は効くが具体例は薄い。**増やすとき**: `presets.rs` に 1 件足すだけ（一覧 UI は 2 件目から要る）
0. **「空から書く」の廃止（D-69、#121）— 2026-09-24 完了**。入口はサイドバーの「題から書く」1 つ。空の状態も同じ。`article.new` は辞書から削除、`host.createArticle` は残す。**残る白紙の道**: 文体ページの「この文体で書く」は今も白紙の記事を作る（消すかは本人の判断待ち）
0. **「題から書く」を本命に（D-68、#118）— 2026-09-24 完了**。本人の「題から書くほうが本命」。B（D-67）は日常の主が白紙に見えたため A へ改訂: サイドバーの主ボタン「題から書く」、副「空から書く」（「新しく書く」の語は廃止）、一覧の頭の帯は削除。`quietNew` は同じ規則。懸念: 記事を開いている画面でサイドバーの「題から書く」がミントのまま本文の「書き直す」と並ぶ（広げるなら `quietNew` を記事の画面にも）。記録 `ux/2026-09-24-topics-path.md` の「改訂（A へ）」
0. **「題を出す」までの導線（D-67、#115）— 2026-09-24 完了**。lita-ux のレビュー（入口 2 つの重さが逆・検索欄の隣は道具に見える・1 つの道に 3 つの名前・初回に白紙だけ・主ボタン 2 つ）→ 3 案 → 本人が B。見出し行の小ボタンは消し、一覧の頭にミントの一行「題から書く」（文体が無くても出す、#112 維持）、記事 0 の空の状態は主ボタン「題から書く」＋薄い「新しく書く」。本文が主ボタンを持つ間はサイドバーの「新しく書く」を白に落とす（`quietNew`）。記録 `ux/2026-09-24-topics-path.md`
0. **起動画面と「題を出す」の導線（#111、#112）— 2026-09-24 完了**。起動時の素の一文を `Launch.tsx`（ロゴ「Lita.」の点が明滅、一言、何を待っているかの一文、3 秒超で「時間がかかっています」。A 案「紙に浮かぶ」。B 案「二段階が進む」は `design/launch-b.html` に残る）に。「題を出す」は文体が無くても隠さず、押すと `TopicPicker` の案内（まず文体を作りましょう）。記録 `ux/2026-09-24-launch.md`
0. **題材の語の雲（D-66、#108）— 2026-09-24 完了**（要件 `requirements/2026-09-24-topic-cloud.md`（必須 18）、UX の 3 案と実装記録 `ux/2026-09-24-topic-cloud.md`、試行 `poc/article-queue/cloud-trial.md`）。「書く題を決める」の最上部に「よく書いている言葉」（A 案: 雲が先）。Rust: `topics::{cloud_prompt, cloud_schema, tidy_cloud, TopicCloud, CloudWord}`、コマンド `get_topic_cloud` / `gather_topic_cloud`（中止は `cancel_generate`）、`suggest_topics` と `enqueue_articles` に `subjects`（題のプロンプトとブリーフに「題材」の行）。DB: `user_settings.topic_cloud jsonb`（migration `20260924100000_topic_cloud.sql`、本番適用済み）。編集方針の「よく書く題材」は画面と叩き台から外した（`Policy.topics` は旧データ用に残る）。**未検証**: 実機での自動取得と拾い直し
0. **セッションの自動更新と「サインインし直す」（#105）— 2026-09-24 完了**。アクセストークンは起動時にしか更新されず 1 時間で切れていた。`AppState::refresh_if_stale`（残り 5 分を切ったら refresh_token で更新、キーチェーンに保存）を `access_token()` と 60 秒ごとの常駐スレッドから呼ぶ。切れた後の導線: `ErrorNote` が `session_expired` / `not_signed_in` のときだけ「サインインし直す」を出し（`lita:sign-in` イベント）、`App.tsx` が `signIn()` を回す
0. **推論量は常に最大・ブリーフの提案・余白（D-63、D-64、#96、#97）— 2026-09-24 完了**（要件 `requirements/2026-09-24-editor-brief.md`、UX 記録 `ux/2026-09-24-editor-brief.md`）。「考える量」は廃止し記事の生成は常に medium（`Effort::Quality`。high は本人が同日に Medium へ戻した。`Effort::Best` は残るが未使用）。エディタに「別の案を出す／ブリーフを提案する」（`suggest_brief`、`topics::brief_prompt`、medium、中止は `cancel_generate`）、題が空なら押せない、置き換え直後だけ「前のブリーフに戻す」、順番待ち由来の「長さ」「調べず」の行は残す。長文の出す先では書いている間に「1〜3 分ほど」の一文。右の欄の余白は `--gap` 一段＋札と部品の間 6 に整理。**懸念**: ブリーフ欄が 4 行で提案文の後半が隠れる（欄を伸ばすかは本人に画を見せて決める）
