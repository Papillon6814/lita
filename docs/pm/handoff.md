...
1. **note の公開記事の取り込み（B-20 / #19）** — アカウント名 → 公開 API で記事一覧と本文 → 貼り付け画面に候補として流す
2. **Medium（B-43）、X アーカイブ（B-44）** — 同じ「候補として流す」口に載せる — v0.1 の入口。文章を受け取って `VoiceProfile::extraction_prompt` に渡す
