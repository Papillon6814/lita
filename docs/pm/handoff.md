...
1. **貼り付け／ファイル取り込みの UI（B-12 / #12）** — 文章を受け取り `VoiceProfile::extraction_prompt` で Codex に抽出させ、`lita-store::UserStore::create_voice` に流す。抽出中はステージ表示（B-14） — v0.1 の入口。文章を受け取って `VoiceProfile::extraction_prompt` に渡す
2. **ブリーフ → 生成 → 承認 → コピーの UI（B-07 / #13）**、速度／品質の切り替え（B-13 / #14）、Voice 生成のステージ表示（B-14 / #15）
