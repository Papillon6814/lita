...
16. **実装後の UX 再レビュー（#38）**。ブラウザ用モックで全 15 場面を撮り、8 件を指摘して全部反映（`docs/pm/2026-09-22-ux-rereview.md`）。下書きを最上部に、コピー後の状態、⌘⏎、戻るで中止、など（PR #27、#26）。Google はデスクトップ用クライアントでもシークレットを要求すると確定し、Supabase 経由の OAuth に切り替えた（D-41）。`crates/lita-auth` がログインとリフレッシュを担う
...
1. **未ログイン・利用上限の実地再現（B-11 / #17）** — stderr の文字列マッチを確かな判定に置き換える
2. **Voice の一文の精度（B-45 / #39）** — 長文・英語の Voice で不自然にならないか
3. 公開前の運用: Google 同意画面を Testing → 本番へ。未使用の「Lita desktop」クライアントは削除候補
