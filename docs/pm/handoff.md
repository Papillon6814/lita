...
1. **約30秒の待ち時間をどう扱うか決める（B-10 / D-19 / Issue #3）** — ストリーミングが使えないので生成中は無音になる。**UI を作る前にここを決めないと手戻りする**
2. **Voice スキーマの確定（B-04）** — `corpus` バイナリの出力を叩き台に、各フィールドへ `description` を付ける。`description` を書いた版はすでに `corpus.rs` にある
3. **Tauri v2 スキャフォールド（B-05）** — 上記2つが固まってから
4. **貼り付け／ファイル取り込みの UI（B-12）** — v0.1 の入口
