...
| 貼り付け／ファイル取り込みの UI | 完了（#12）。ステージ表示込み。Voice カードで抽出結果を表示 |
| note / Medium / X の取り込み | 完了（PR #35）。取り込み画面のセグメントから |
| UI/UX | 完了（PR #36）。見本どおりの実装。Voice 画面は人物紹介＋畳んだ詳細（行ごとに修正可）、取り込みはタイルと固定バー |
