...
- **最終更新**: 2026-09-22
- **フェーズ**: 実装。ただし **2026-09-22 に保存方式が SQLite からサーバへ転換**（D-36〜D-39）。バックエンド基盤の選定待ち
...
| --- | --- |
| プロダクトの方向性の合意 | 完了 |
| 主要な技術選定 | 完了（Tauri v2 / Codex CLI サブプロセス / ~~SQLite~~ → サーバ（基盤未決）/ MIT） |
...
| 項目 | 状態 |
| --- | --- |
| バックエンド基盤の選定 | **調査完了、本人の決定待ち**（推奨 Supabase） |
| Google ログイン | 未着手 |
