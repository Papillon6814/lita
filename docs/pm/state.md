...
- **最終更新**: 2026-09-23
- **フェーズ**: v0.2 の 3 柱（文体・記事・エディタ）を実装済み（D-44〜D-46、#49、PR #50〜#55）。本人のドッグフーディング中。配布（#25）は本人用の一周（GitHub Releases ＋ アプリ内更新、v0.2.2）まで完了。文体機能は UX レビューを受けて簡素化済み（D-53〜D-56）。Apple 署名と Homebrew は保留（D-51）。文体は根拠つき（D-59）、材料の入口は 2 つの器（D-60）、**記事はまとめて生成できる（D-61: 題の提案・編集方針・順番待ち、v0.2.12）**。ただし **2026-09-22 に保存方式が SQLite からサーバへ転換**（D-36〜D-39）。基盤は Supabase（D-40）
...
| ブリーフ → 生成 → 承認 → コピー | 完了（#13 / #14 / #16）。速度／品質の切り替え、送信内容のプレビュー、中止つき。**v0.1 の完成定義が一周した（2026-09-22、本人確認）** |
| v0.2: 記事一覧・エディタ（自動保存・版履歴・長文）・Voice 複数と試し書き | 完了（2026-09-23、PR #50〜#55）。モックで全場面を確認。実操作の確認は本人待ち |
